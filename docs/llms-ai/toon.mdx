  ...
  const convertToToon = () => {
    try {
      ...
      if (data.users && Array.isArray(data.users)) {
        ...
        const toon = `users[${users.length}]{${fields.join(',')}}:\n${users.map(u => 
          `  ${fields.map(f => u[f]).join(',')}`
        ).join('\n')}`;
        setToonOutput(toon);
      } else {
        setToonOutput('Format JSON non supporté pour cet exemple. Utilisez un objet avec un tableau "users".');
      }
    } catch (e) {
      setToonOutput('Erreur : JSON invalide');
    }
  };
